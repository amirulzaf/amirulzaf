  <?php
   ...
   if($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $sql = "SELECT User_ID,User_name,User_email,User_course,User_contact,User_Type FROM user WHERE  User_ID= '$myusername' and User_Pass ='$mypassword'";
      $result = mysqli_query($db,$sql);
      $row = mysqli_fetch_array($result,MYSQLI_ASSOC);
      
         setcookie( "cooStdId", $row["User_ID"], $date_of_expiry, "/" ) ;
         setcookie( "cooName", $row["User_name"], $date_of_expiry, "/" ) ;
         setcookie( "cooEmail", $row["User_email"], $date_of_expiry, "/" ) ;
         setcookie( "cooCourse", $row["User_course"], $date_of_expiry, "/" ) ;
          setcookie( "cooContact", $row["User_contact"], $date_of_expiry, "/" ) ;
          setcookie( "cooType", $row["User_Type"], $date_of_expiry, "/" ) ;
      ...
      if($count == 1) {
          if($row['User_Type'] == "admin"){
            $_SESSION["admin_sess"] = $row["User_ID"];
            $_SESSION["admin_loggedin"] = 1;
            header("location: Main_pageAdmin.php");
          }else if($row['User_Type'] !== "Admin"){
            $_SESSION["student_sess"] = $row["User_ID"];
            $_SESSION["loggedin"] = 1;
...
body {
    background: linear-gradient(rgba(246, 247, 249, 0.8), rgba(246, 247, 249, 0.8)), url(img/img2.jpg)  no-repeat center center fixed;
...
$(".btn").click(function() {
$(".form-User").toggleClass("form-User-left");
$(".form-Admin").toggleClass("form-Admin-left");
$(".Admin-inactive").toggleClass("Admin-active");
$(".User-active").toggleClass("User-inactive");
        ...
        $('#sgform').submit(function(e) { 
            ...
            $.ajax({
                ...
                success     : function(data){
                        swal(
    'Good job!',
  'Thanks you For Userg Up,You may now Login to System ',
  'success' 
    )
                 },
            })
          document.getElementById("sgform").reset();
                            ...
                            <div class="container">
                             <span class="login100-form-title p-b-34 p-t-27" >
                                <img src="img/logo.png"  width="252" height="84">
                             </span>
    <div class="frame">
        <div class="nav">
            <ul class="links">
                <li class="User-active"><a class="btn">User</a></li>
        ...
        <div ng-app ng-init="checked = false">
            <form class="form-User" action="" method="post" name="form"> <label for="username">Userid</label> <input class="form-styling" type="text" name="name" placeholder="" required /> <label for="password">Password</label> <input class="form-styling" type="password" name="password" placeholder="" required /> <input type="checkbox" id="checkbox" /> <label for="checkbox"><span class="ui"></span>Keep me signed in</label>
                <div class="btn-animate" > <button class="btn-User" type="submit" name="submit">Sign in</button> </div>
            </form>

           <form class="form-Admin" action="" method="post" name="form"> <label for="username_Admin">Userid</label> <input class="form-styling" type="text" name="name" placeholder="" required /> <label for="password">Password</label> <input class="form-styling" type="password" name="password_Admin" placeholder="" required /> <input type="checkbox" id="checkbox" /> <label for="checkbox"><span class="ui"></span>Keep me signed in</label>
                <div class="btn-animate" > <button class="btn-User" type="submit" name="submit">Sign in</button> </div>
            </form>
